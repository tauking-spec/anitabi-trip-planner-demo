...
- 支持导出 KML 文件，并按每日路线分层，可在 Google My Maps 中导入。
- 支持分享链接，编码作品 ID、起点、半径、天数、每日点数、时间窗口、强度和路线模式。
- 已选作品会异步显示 Anitabi 是否有巡礼点；未收录作品以红色卡片提示。
- 支持将当前搜索结果一键全选加入作品集合，并一键移除未收录作品或清空已选作品。
- 支持路线强度、每日时间窗口、“只看高密度区域”和“环路路线”模式。
